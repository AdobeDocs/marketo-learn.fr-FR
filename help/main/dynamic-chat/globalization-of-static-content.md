---
title: Globalisation du contenu statique
description: Globalisation du contenu statique.
role: User
level: Beginner
doc-type: Feature Video
last-substantial-update: 2023-03-20T00:00:00Z
jira: KT-12966
thumbnail: 3416924.jpeg
exl-id: 19ccc619-50f9-4737-875c-8f87f55a0183
feature: Dynamic Chat
source-git-commit: 096d4b42008446a72f92b8fe509c0c216bc8f904
workflow-type: tm+mt
source-wordcount: '79'
ht-degree: 10%

---

# Globalisation du contenu statique

Vous savez peut-être déjà comment créer une boîte de dialogue non anglaise (si ce n’est pas le cas, [cliquez ici](https://nation.marketo.com/t5/dynamic-chat-discussion/design-non-english-language-conversations-in-dynamic-chat/m-p/324317#M39){target="_blank"}). Vous pouvez personnaliser le contenu statique/système dans la langue de votre choix.

>[!VIDEO](https://video.tv.adobe.com/v/3439216/?captions=fre_fr&quality=12&learn=on){transcript=true}

## Vidéos plus utiles

* [Ajouter et gérer des utilisateurs de &#x200B;](user-management.md)
* [Créer et gérer des dialogues personnalisés](dialogue-management.md)
* [Activer votre intégration Marketo avec [!UICONTROL Dynamic Chat]](marketo-integration.md)
